...
export default function CartDrawer() {
  ...
  return (
    ...
    >
      ...
      <aside
        className={`absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-paper-raised shadow-soft transition-transform duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-line px-6 py-5">
          ...
          <button onClick={close} aria-label="Close cart" className="text-ink-dim hover:text-ink">
        ...
        {items.length > 0 && (
          <div className="border-b border-line px-6 py-4">
            ...
            <div className="h-1 w-full overflow-hidden rounded-full bg-paper-soft">
              <div
        ...
        <div className="flex-1 overflow-y-auto px-6">
          {items.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-4 text-center">
              <p className="font-serif text-2xl text-ink-dim">Your cart is empty</p>
              ...
              {items.map((item) => (
                <li key={item.id} className="flex gap-4 py-5">
                  <Link
                    href={`/shop/${item.id}`}
                    onClick={close}
                    className="relative size-20 shrink-0 overflow-hidden rounded bg-paper"
                  >
                    {item.product.images[0] ? (
                      ...
                      <span className="absolute inset-0 flex items-center justify-center font-serif text-xl text-ink-faint">
                        {item.product.mukhi ?? "✦"}
                      </span>
                    )}
                  </Link>
                  ...
                  <div className="flex flex-1 flex-col">
                    ...
                    <p className="text-xs text-ink-faint">{item.product.mukhiLabel}</p>
        ...
        {items.length > 0 && (
          <div className="border-t border-line px-6 py-5">
            ...
            <p className="mt-3 text-center text-[0.65rem] tracking-wide text-ink-faint">
