  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 md:py-12">
        {/* Breadcrumb */}
        <nav className="mb-8 flex items-center gap-2 text-xs uppercase tracking-[0.16em] text-bone-faint">
          <Link href="/shop" className="hover:text-gold-soft">Shop</Link>
          <span>/</span>
          <Link href={`/shop?category=${product.category}`} className="hover:text-gold-soft">
        ...
        <div className="grid gap-10 md:grid-cols-2 md:gap-14 md:items-start">
          ...
          <div className="md:py-4">
            ...
            <p className="mt-4 text-lg leading-relaxed text-bone-dim">{product.tagline}</p>

            {/* Price */}
            <div className="mt-6 flex items-center gap-3">
              <span className="font-serif text-3xl text-bone">{inr(product.price)}</span>
              {product.mrp > product.price && (
                <>
                  <span className="text-lg text-bone-faint line-through">{inr(product.mrp)}</span>
                  <span className="rounded-full bg-gold px-2.5 py-1 text-[0.65rem] font-medium uppercase tracking-wide text-ink">
                    {off}% off
                  </span>
                </>
              )}
            </div>
            <p className="mt-1 text-xs text-bone-faint">Inclusive of all taxes</p>
            ...
            {/* Description */}
            <div className="mt-8">
              <p className="leading-relaxed text-bone-dim">{product.description}</p>
            </div>
            ...
            <div className="mt-8">
              ...
              <ul className="space-y-3">
                {product.benefits.map((b) => (
                  <li key={b} className="flex items-start gap-3 text-sm text-bone-dim">
...
function Assurance({
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span className="text-[0.7rem] tracking-wide text-bone-dim">{label}</span>
    </div>
  );
}
